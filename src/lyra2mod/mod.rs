...
pub fn lyra2mod(
    ...
    for _tau in 1..=time_cost {
        ...
        while !row0 {
            ...
            //------------------------------------------------------------------------------------------
            //rowa = ((uint64_t) (state[index % 16])) % nRows; //(USE THIS FOR THE "GENERIC" CASE)	//(USE THIS IF n_rows IS A POWER OF 2)
            index = state[index as usize % 16];
            rowa = state[index as usize % 16] % n_rows as u64; //(USE THIS FOR THE "GENERIC" CASE)
            //------------------------------------------------------------------------------------------
